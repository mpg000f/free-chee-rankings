...
- **The navbar is full.** `.navbar-inner` is capped at `max-width: 1200px`, so with 12 links there is zero horizontal slack — a 13th link, or lengthening any label, silently clips the last item at *every* desktop width (the nav scrolls, hiding it). Nav labels are already abbreviated relative to page headings ("Records" → "League Records", "Lookback" → "4-Year Lookback", "Waivers" → "Waivers & Trades", "Stats" → "Owner Stats"). Measure with `navLinks.scrollWidth - navLinks.clientWidth` before adding anything.
