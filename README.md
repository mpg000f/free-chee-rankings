...
- **The navbar collapses below 1240px** (`site/js/nav-menu.js` + the mobile-nav and nav-group blocks in `style.css`). Under that width every link goes behind a toggle, which is what makes the site usable on a phone — before it, only 4 of 12 links were reachable. `BAR_FITS_ABOVE` in the JS and the `1239px`/`1240px` media queries in the CSS must always be changed together.
- **Draft Value, Draft History, Waivers and Trade Grades sit under one "Draft & Moves" item** — a dropdown on the wide bar, an expanding section inside the mobile panel, same markup either way. That took the top level from 12 items to 10 and left roughly 60px of slack at 1240px where there used to be none, so there is room for another link now. Adding one still means editing every page plus the hardcoded template in `build_owner_share_pages.py`; a throwaway script over both trees is the sane way to do it.
