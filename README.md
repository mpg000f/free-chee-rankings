...
- **Consolation games are postseason but not playoffs.** `CONSOLATION_ROUNDS` in `build_engagement_data.py` covers the losers' half of the bracket — Consolation Semifinal, 5th and 7th Place Games — and sets a `consolation` flag on each game, so Head-to-Head shows its "PO" badge only for real playoff meetings. The 3rd-place game is deliberately excluded from that set: both teams got there by winning a playoff game, which is the same line `schedule.js` draws when it dims a round. The flag is derived from the round label, so it has to be computed after `assign_playoff_rounds`; a season whose labels were dropped keeps every postseason game flagged as a playoff, since overstating a placement game beats quietly demoting a semifinal. Hovering a week in the H2H log names the exact round either way.
